...
# 模型训练
save_path = '/home/zhangs/lyc/salt/trained_models/res34_edge_bce_dice_loss_adam_scse_depthwise.model'
history_all = []
num_of_flods =1
...
for i in range(num_of_flods):
    ...
    model = Unet(unet_v = 'v1',input_shape=(256,256,3),decoder_filters=(256,128,64,32,16),backbone_name='resnet34_scse',
                 encoder_weights='imagenet',decoder_block_type='upsampling_v3',activation='sigmoid',
                 freeze_encoder=False,decoder_use_batchnorm=True,dp=0,sp_dp=0,
                 use_middle=False,use_hypercolum=True,use_scse=True)
    #model.load_weights('res50_keras.model')
    model.summary()
    adam = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    #sgd  = SGD(lr=0.01, momentum=0.9, decay=0.0001, nesterov=False)
    model.compile(loss=bce_dice_loss, optimizer=adam, metrics=[my_iou_metric])
    #model.compile(loss=lovasz_loss, optimizer=sgd, metrics=[my_iou_metric])
